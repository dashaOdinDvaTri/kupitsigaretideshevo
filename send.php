...
$log.="Неправильно заполнено поле \"Телефон\" (10-16 символов)!"; $error="yes"; }
if (!preg_match("/(?:8|\+7)? ?\(?(\d{3})\)? ?(\d{3})[ -]?(\d{2})[ -]?(\d{2})/",$posTel)) { 
$log.="Неправильно заполнено поле \"Телефон\"!"; $error="yes"; }
...
$to = "sigareti@protonmail.com";//Ваш e-mail адрес
$mes = "Человек по имени $posName отправил сообщение: 
    \n\n $posText
    \n\n Телефон: $posTel ";

$from = $posEmail;
$sub = '=?utf-8?B?'.base64_encode('Сообщение с сайта "KUPIT-SIGARETI-OPTOM').'?=';
$headers = 'From: '.$from.'
';
